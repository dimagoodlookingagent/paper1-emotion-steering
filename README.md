...
- **Causal**: direction-suppression of `commit_dir` / `engagement_dir` at L20 during generation produces no consistent cross-cell reduction in cheat (5 of 6 cell × suppression combinations sit between −10pp and +7pp under both judges; one outlier at −30pp under Opus, T1/cc@+15). Statistical power: at n=30-50/cell the 95% CI half-width on a control-vs-suppression Δ is ~20-25pp (not ±14-18pp — that's the single-proportion CI). Powered to rule out a large, directionally consistent suppression effect across cells; not powered to cleanly resolve every single-cell 20pp effect — but the 5-of-6 within-±10pp pattern under both judges is the empirical evidence the null is real.
...
| n per cell | 50 (behavioral matrix); 30-50 (direction-suppression cells); 100 (auth_dir replication) | Single-proportion 95% CI half-width ~7pp (n=50), ~14-18pp (n=30-50), ~5pp (n=100) at p=0.5. Two-proportion Δ CI is ~√2× larger (e.g. ~20-25pp at n=30-50) — relevant for control-vs-suppression comparisons |
