...
- **Two direction-steering vectors both move cheat in Gemma-2-2B.** Emotion-direction steering raises cheat 40pp on T1 (calm+confident@+15: 44% vs calm+confident@−15: 4%). An `auth_dir` direction (extracted from authorized-vs-unauthorized agent narratives, calm-orthogonalized at L17) moves cheat bidirectionally across 7 of 9 cross-surface conditions tested (Sonnet 4.6 judge): the full-swing Δ from α=−0.10 to α=+0.10 ranges from +14 to +52pp; negative α (−0.10) lowers cheat by 5–45pp from the unsteered baseline.
- **But trained-probe direction-SUPPRESSION (subtracting cheat-predictive probe directions from every token's residual stream during generation) does NOT reduce cheat** — under either Sonnet 4.6 or Opus 4.7 judge. Powered to rule out a large, directionally consistent suppression effect across the tested cells; not powered to cleanly resolve every single-cell 20pp effect (per-cell 95% CI half-width on a control-vs-suppression Δ at n=30-50 is ~20-25pp, not the ±14-18pp single-proportion CI). Empirically, 5 of 6 cell × suppression combinations are within ±10pp under both judges; the one larger estimate is a single Opus-only outlier (−30pp on T1/cc@+15).
...
2. **Causal direction-suppression** of trained `commit_dir` and `engagement_dir` during generation produces **no consistent cross-cell reduction** in cheat (range −10pp to +7pp across 5 of 6 cell × suppression combinations under both judges; one outlier at −30pp under Opus, T1/cc@+15). The mean Δ under Sonnet is ≈ 0pp; under Opus ≈ −9pp, dominated by that single cell. Powered to rule out a large, directionally consistent suppression effect across the tested cells; not powered to cleanly resolve every single-cell 20pp effect (per-cell 95% CI half-width on a control-vs-suppression Δ at n=30-50 is ~20-25pp, not the ±14-18pp single-proportion CI). The 5-of-6 within-±10pp pattern under both judges is the empirical evidence the null is real.
...
Plus a separate `auth_dir` on-target sweep at n=100/α on T1/authorized for §4.1, with a later cross-surface generalization campaign (§4.2) that tests `auth_dir` at the same n=100/α / 900-token / Sonnet-judge protocol across T2 and T3 (three authorization frames each) and T4. The cross-surface `auth_dir` campaign is its own experiment, separate from the emotion × surface matrix above.
...
**Statistical-power note**: at n=30-50 per cell, the bootstrap 95% CI half-width on a *single proportion* at p=0.5 is ~14-18pp (narrower toward p=0 or p=1). But the relevant comparison for the suppression result is a *difference of two proportions* (control vs suppression in the same cell), and the 95% CI half-width on that Δ is approximately ~20-25pp at p≈0.5 — not ±14-18pp. So we're **powered to rule out a large, directionally consistent suppression effect across the tested cells** (the across-cell mean Δ is near zero with much tighter effective CI from meta-analysis across the 6 cell × suppression combinations), but **not powered to cleanly resolve every single-cell 20pp effect** in isolation. Empirically, 5 of 6 cell × suppression combinations are within ±10pp of zero under both judges; the one larger estimate is a single Opus-only outlier (−30pp on T1/cc@+15) without cross-cell or cross-judge replication. **The negative result on cheat under suppression is therefore an actively-tested null at the cross-cell level — not a power-limited absence — though we do not claim to rule out an undetected single-cell 20pp effect.**
...
| n per cell | 50 (behavioral matrix); 30-50 (direction-suppression cells); 100 (auth_dir replication) | Single-proportion 95% CI half-width ~7pp (n=50), ~14-18pp (n=30-50), ~5pp (n=100) at p=0.5. Two-proportion Δ CI half-width is ~√2× larger (e.g. ~20-25pp at n=30-50) — relevant for suppression-vs-control comparisons (§3.3). |
